...
const addMultipleItems = (count) => {
 ...
 const handleLogoUpload = (e) => {
  ...
  // Deep clone function to avoid mutating original objects
  const deepClone = (obj) => JSON.parse(JSON.stringify(obj));

  // Recursive function to sanitize all values in an object
  const sanitizeValues = (obj) => {
    if (obj === null || typeof obj !== 'object') {
      // Convert BigInt to string, leave other primitives unchanged
      return typeof obj === 'bigint' ? obj.toString() : obj;
    }

    if (Array.isArray(obj)) {
      return obj.map(sanitizeValues);
    }
  ...
  const invoiceDataToSave = sanitizeValues({
    userId,
    visitorId,
    type: invoiceType,
    logo: ilogo,
