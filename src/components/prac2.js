...
const invoiceType = getInvoiceType(location.pathname);
  ...
  // Add this to your signinHomepage component
useEffect(() => {
  ...
  if (invoiceNumber) {
    const fetchInvoice = async () => {
      try {
        ...
        if (data.invoice) {
          // Set all your form fields with the invoice data
...
if (!billTo?.trim()) {
  ...
  // 2. Prepare line items with required 'rate' field
  const validLineItems = items
    .filter(item => item.description?.trim())
    .map(item => ({
      description: item.description.trim(),
      quantity: Number(item.quantity) || 1,
      rate: Number(item.price) || 0,  // Using 'rate' instead of 'price'
 ...
 if (validLineItems.length === 0) {
  ...
  // 4. Build the final invoice data object
  const invoiceData = {
  ...
  console.log("Submitting invoice:", invoiceData); // Debug log
  ...
  try {
    const response = await fetch("http://localhost:5000/invoice", {
      ...
      body: JSON.stringify(invoiceData),  // Using invoiceData instead of invoiceDataToSave
    });
